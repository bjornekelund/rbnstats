#!/usr/bin/env bash
# Uses saved analysis results from the last ten days
# to assemble a table sorted by skimmer callsign
# createwebdata creates the required result files.
# These have be to manually created before the first run.
#set -x

RBNFOLDER="rbndata"
WEBFOLDER="webfiles"
OUTFILE=$WEBFOLDER/rbnstats.txt
HISTFILE=$RBNFOLDER/history.txt

test -e "$WEBFOLDER" || mkdir $WEBFOLDER

INFILES="$RBNFOLDER/`date -u --date="1 days ago" +%Y%m%d`.txt $RBNFOLDER/`date -u --date="2 days ago" +%Y%m%d`.txt \
 $RBNFOLDER/`date -u --date="3 days ago" +%Y%m%d`.txt $RBNFOLDER/`date -u --date="4 days ago" +%Y%m%d`.txt \
 $RBNFOLDER/`date -u --date="5 days ago" +%Y%m%d`.txt $RBNFOLDER/`date -u --date="6 days ago" +%Y%m%d`.txt \
 $RBNFOLDER/`date -u --date="7 days ago" +%Y%m%d`.txt $RBNFOLDER/`date -u --date="8 days ago" +%Y%m%d`.txt \
 $RBNFOLDER/`date -u --date="9 days ago" +%Y%m%d`.txt $RBNFOLDER/`date -u --date="10 days ago" +%Y%m%d`.txt"

#echo "\$INFILES="$INFILES

# Produce header of output file
# Print data in reverse chronological order with newest data to the left

cat $INFILES > $HISTFILE

awk '{
  array[$1][$2] = $3;
  call = $1;
}
END {
# Find a call that has been active all ten days
  for (trycall in array) {
    if (isarray(array[trycall])) {
      k = 0;
      for (elem in array[trycall])
        k++;
      if (k == 10) {
        call = trycall;
        break;
      }
    }
  }
  printf("Skimmer  ");
  j = 0
  for (datestring in array[call]) {
    date[j++] = strftime("%m%d", (datestring - 1) * 86400);
  }
# Print in antichronological order
  for (j = 9; j >= 0; j--) {
    printf("%6s", date[j]);
  }
  printf("\n---------------------------------------------------------------------\n");
}' $HISTFILE > $OUTFILE

# Produce meat of output file
# Print data in reverse chronological order with newest data to the left
awk '
{
  array[$1][$2] = $3;
  call = $1;
}
END {
# Find a call that has been active all ten days
  for (trycall in array) {
    if (isarray(array[trycall])) {
      k = 0;
      for (elem in array[trycall])
        k++;
      if (k == 10)
        call = trycall;
    }
  }
# Put the date of the last ten days in date[]
  j = 0
  for (datestring in array[call]) {
    date[j++] = datestring;
  }
  for (i in array) {
	printf("%-9s", substr(sprintf("%s", i), 1, 9));
    if (isarray(array[i])) {
# Print data in row in antichronological order
      for (j = 9; j >= 0; j--) {
        printf("%6s", array[i][date[j]]);
      }
	  printf("\n");
    }
}}' $HISTFILE | sort >> $OUTFILE

echo >> $OUTFILE
echo "Last updated "`date -u "+%F %T"`" UTC" >> $OUTFILE

echo "Spot count analysis result saved in" $OUTFILE
exit
